...
def plot_predictions(
    ...
    start = -grid_size // 2
    labels = range(start, start + grid_size + 1)
    ...
    hm.set(xlabel=f"{direction} direction 1", ylabel=f"{direction} direction 2")

    # 11 ticks including 0 in middle, so step is grid_size / 10
    tick_locator = ticker.FixedLocator(range(grid_size + 1), nbins=11)
    hm.axes.xaxis.set_major_locator(tick_locator)
    hm.axes.xaxis.set_ticklabels(labels)
    hm.axes.yaxis.set_major_locator(tick_locator)
    hm.axes.yaxis.set_ticklabels(labels)
    ...
    if display_ims:
        ...
        def get_ax2_title(im, orig_im):
            total_diff = np.abs(im.astype(np.int32) - orig_im_arr).sum()
            return f"Pixel value diff (abs) to original.\nTotal={total_diff}"

        ax2.set_title(get_ax2_title(im=orig_im_arr, orig_im=orig_im_arr))
        ...
        def onclick(event):
            if ax0.contains(event)[0]:
                ...
                ax2.set_title(get_ax2_title(im=im, orig_im=orig_im_arr))
    ...
    ax3.xaxis.set_major_locator(tick_locator)
    ax3.xaxis.set_ticklabels(labels)
    ax3.yaxis.set_major_locator(tick_locator)
